...
// {
	...
	if($_GET['type'] == 'hosts' && isset($_GET['artistId']))
	{
	
		$getVideos1 = mysql_query("SELECT * FROM uploaed_video WHERE `user_id` = '$_GET[artistId]'  AND `user_type` = 'club' AND `featured` = '1' ORDER BY `track_no` ASC");
	  ...
	  <div class="home_content_bottom mbottom20" style="background:none;">
		<div class="wrapper">
		  ...
		  <div class="MySittiplaylist newPlayList artistDetailList">
			<ul id="playlist">
			  <h1><?php //echo $_GET['category'];?> <?php 
					$clubname = mysql_query("SELECT club_name FROM `clubs` WHERE `id` = '$_GET[artistId]'");
					$cnm = mysql_fetch_assoc($clubname);
					echo $cnm['club_name'];
	...
	?>
		...
		?>
	  	<div class="TonightArtistListing">
			<h2> 
				<img src="images/artist.png" alt="" />Local Artist
			</h2>
			...
			<?php 
					...
					// {
						...
						// echo "SELECT `ev`.`id` as `eventid`,`ev`.`event_image_thumb`,`c`.`id`,`cc`.`city_name`,`z`.`name` as `zonename`,`c`.`image_nm`,`c`.`club_name` as `profilename` FROM `clubs` as `c`, `capital_city` as cc, `zone` as `z`, `events` as `ev`
						// WHERE `c`.`club_city` = `cc`.`city_id`
						// AND `c`.`club_state` = `z`.`zone_id`
						// AND `cc`.`state_id` = `z`.`zone_id`
						// AND `c`.`id` = 	`ev`.`host_id`
						// AND `profileType` = 'Public' 
						// /*AND `c`.`club_city` = '$_SESSION[id]'*/
						// AND `c`.`non_member` = '0'
						// AND `c`.`type_details_of_club` LIKE '%$_GET[category]%'
		...
		<div class="band_event_list tv_event_list fullblack NowStreamingListing">
			<h2> <img src="images/streaming-now.png" alt="" /><?php echo $cnm['club_name']; ?> - Events</h2>
			<!-- <form method="post">
			...
			<div class="img_slider_btm">
		  		...
		  		<?php
		  			$getEvent = mysql_query("SELECT * FROM forum WHERE user_id = '$_GET[artistId]'");
						if(mysql_num_rows($getEvent) > 0)
						{ ?>
						<ul class="hostSlider" id="streamingHosts">
						<?php while($event = mysql_fetch_assoc($getEvent))
							{ ?>
								<li class="custom_slide"> 
									...
									<?php if(empty($event['image_thumb']))
										...
										<img src="<?php echo $SiteURL.$event['image_thumb'];?>" alt=""> 
									 </a>
									<div class="live_stream_new"> </div>
									<span class="name_users">
						...
						<?php } else { ?>
							...
							<li class="not-found">Event not found</li>
