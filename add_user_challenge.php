...
$titleofpage="Add User Challenge";
...
if(!isset($userID)){ $Obj->Redirect($SiteURL); }
	...
	$cndg="cg.group_type='private' AND cg.create_by='".$_SESSION['user_id']."'";

	$groups_lst = "select cg.*,c.city_name,u.first_name,u.last_name,u.image_nm from chat_groups as cg left join capital_city as c on(cg.city_id=c.city_id) left join user as u on(u.id=cg.create_by) where ".$cndg." ";

	

// get user groups 

  // $get_groups=mysql_query("select g.group_name,g.id from  chat_users_groups as cgs join chat_groups as g on(g.id=cgs.group_id) where g.create_by='".$_SESSION['user_id']."'");

  $get_groups = mysql_query($groups_lst);
...
$friends_list = "select distinct(fs.friend_id),fs.friend_type, fs.user_type,fs.status as freindstatus,fs.chat,fs.id as f_id from friends as fs where fs.user_id='".$_SESSION['user_id']."' AND fs.friend_id != 1 AND fs.status = 'active' AND fs.friend_type = 'user' GROUP BY friend_id ORDER BY id ASC";



  // $get_friends=mysql_query("select u.first_name,u.last_name,u.id from   friends as f join user as u on(u.id=f.friend_id) where f.user_id ='".$_SESSION['user_id']."' group by f.friend_id");

  

  $get_friends = mysql_query($friends_list);
			...
			.autocomplete({
			   ...
			   <?php 
				  ...
				  while($rs2 = mysql_fetch_assoc($get_friends)) {
					  ...
					  $get_profilename = mysql_query("SELECT profilename FROM user WHERE id = '".$rs2['friend_id']."'");

					  $f_profilename = mysql_fetch_assoc($get_profilename);

					  

					  $current_profilename = mysql_query("SELECT profilename FROM user WHERE id = '".$_SESSION['user_id']."'");

					  $c_profilename = mysql_fetch_assoc($current_profilename);

					  

						if($f_profilename['profilename'] != "" && $f_profilename['profilename'] != " " && !empty($f_profilename['profilename']) && $f_profilename['profilename'] != $c_profilename['profilename']){

						 

						 $val2[$l]['id'] = $rs2['friend_id'];

						 $val2[$l]['label'] = $f_profilename['profilename'];
...

{
 ...
 if(start_date!=""){
	 ...
	 ajaxFunction("https://mysitti.com/admin.....///contest/ChkstartDate.php?start_date="+start_date, function()
...
include('resize-class.php');
...

  {
...

	{
...

		 {
			 ...
			 $get_fds=mysql_query("select f.friend_id from  friends as f where f.user_id ='".$_SESSION['user_id']."' AND friend_type='user' group by f.friend_id");
			 ...
			 $user_main_array = array();
			  ...
			  if(isset($_POST['group']))

				 {
						...
						$group_val = rtrim($_POST['group'],",");

						

						$g_user=mysql_query("SELECT user_id FROM `chat_users_groups` WHERE `group_id` IN ( ".$group_val." ) AND user_id != '0' group by user_id");
...

				 {
					...
					$user_friend_val = rtrim($_POST['friend'],",");

					$friend = explode(",",$user_friend_val);
				...
				if(!empty($group_array) && empty($friend_array)){

					

					$user_main_array[] = array_unique(array_filter($group_array));

					

				}elseif(empty($group_array) && !empty($friend_array)){

					

					$user_main_array[] = array_unique(array_filter($friend_array));
					...
					$user_main_array[] = array_unique(array_filter(array_merge($friend_array, $group_array)));
						   ...
						   foreach($user_main_array[0] as $single_user_id){

							   

								$ValueArray_g = array($single_user_id,'content',$Success,$host_id);	
								...
								mysql_query("INSERT INTO user_notification (`from_user`, `to_user`, `type`, `added_on`, `status`, `common_identifier`, `from_user_type`, `to_user_type`)VALUES('".$host_id."', '".$single_user_id."', 'challenge', '".$challenge_added_on."', '1', '".$c_identifier."', 'user', 'user')");
 ...
 <div id="modal" class="popupContainer v2_adduser_modal">

 <div class="v2_add_user_wrapper">
		...
		<div class="popupBody">
			...
			<div class="user_register">
                    ...
                    <form id="add_contest" name="add_contest" onsubmit="return validate_contest1();"  method="post"  enctype="multipart/form-data">

               			<div>

                          <label>Contest Title:</label>

                        <textarea id = "contest_title" name="contest_title" style="" ></textarea>
                        ...
                        <textarea  name="contest_desc" style=""></textarea>
                       ...
                       <textarea name="contest_rule" style=""></textarea>
                       ...
                       <div class="def_city">
                         ...
                         <?php echo $city_n['city_name']; ?> <a style="color: #1c50b3;" href="javascript:void(0);" onclick="javascript:$('#city_panel').toggle();">[Change City] </a>
                       ...
                       <div id="city_panel" style="display:none;">
                        ...
                        <div class="selctcontry"><label>Country:</label>
		  ...
		  <div id="groups" style="display:none;">
                ...
                <div class="err_reg_v2">Please type first few letters</div>
            ...
            <div id="submit_btn"><input class="button" name="add_HostContest" style="width: ;" type="submit" value="Add Challenge" id="submit3" /> &nbsp;&nbsp;&nbsp;

            <a class="canceluseradd" href="javascript:void(0);"><input style="float:right" class="button" name="cancel" onclick="javacript:self.close();" type="button" value="Cancel"/></a></div>
...
if ($('#datepicker_regi').val() == '') {

 alert("Please enter contest register date");
...
var shout_type = $('input[name=shout_type]:checked', '#add_contest').val();

 

 if (shout_type == 'private') {

  if ($('#search_val').val() == '' && $('#search_val2').val() == '') {

   alert('Please select groups or friends');
