...
function ticket_master_api_call($category, $term, $trigger)
{
	$curl = curl_init();

	curl_setopt_array($curl, array(

	CURLOPT_URL => (empty($trigger)) ? "https://app.ticketmaster.com/discovery/v2/".$category."?apikey=RfcE2uw4xpNzXAWjzvXzA5xxiKuuU9G4&countryCode=US&keyword=".$term."" : "https://app.ticketmaster.com/discovery/v2/".$category."?apikey=RfcE2uw4xpNzXAWjzvXzA5xxiKuuU9G4&countryCode=US&keyword=".$term."&classificationName=".$trigger."",
...
if (!empty($term)) {
	...
	$events = ticket_master_api_call('events', $term, $trigger);
	$events = (count($events) > 0 && isset($events['_embedded']['events']) ) ? $events['_embedded']['events'] : [];

	$venues = ticket_master_api_call('venues', $term, $trigger);
	$venues = (count($venues) > 0 && isset($venues['_embedded']['venues']) ) ? $venues['_embedded']['venues'] : [];

	$attractions = ticket_master_api_call('attractions', $term, $trigger);
