...
else
{
	...
	elseif($_GET['category'] != 'Clubs')
	{
		...
		$fetchID = mysql_fetch_assoc($GETCAT);
		$CATID = $fetchID['id'];
		$getFearuredClubs = mysql_query("SELECT * FROM `clubs` WHERE `profileType` = 'Public' AND `type_details_of_club` LIKE '%$fetchID[name]%' AND  `profile_count` > '1' AND `non_member` = '0' /*AND `club_city` = '$_SESSION[id]'*/ ORDER BY `profile_count` DESC ");
	}
				...
				if(count($videosArray) > 0)
				{
					...
					foreach($videosArray as $rowVideos)
					{
					 if(strip_tags($rowVideos['user_type']) == 'user')
						...
						{
							$getClubInfo = mysql_query("SELECT * FROM `clubs` WHERE `profileType` = 'Public' AND  `id` = '$rowVideos[host_id]'  ");
							$fetchClubInfo = mysql_fetch_assoc($getClubInfo);
	...
	?>
		...
		?>
	  	<div class="TonightArtistListing">
			<h2> 
				<img src="images/artist.png" alt="" />Tonight <?php echo $_GET['category'];//echo $TITLE;?>
			</h2>
				...
				<select id="SelectArtist" onchange="changeresults();">
					...
					<?php 
						if(mysql_num_rows($getSubcats1) > 0)
						{
							...
							{
					?>
								<option  value="<?php echo $rest['name']; ?>"><?php echo $rest['name']; ?></option>
					<?php 
							}
						}
			...
			?>
							<div class="ArtistBox">
								<a class="fullListView line-height" href="<?php echo $SiteURL;?>fullViewPage.php?fullPage=Artists">View All <img src="images/arrow_yellow.png" alt=""></a>
								<div class="clear"></div>
			...
			<?php 			}
				...
				?>
						<div class="ArtistBox">
							<a class="fullListView line-height" href="<?php echo $SiteURL;?>fullViewPage.php?fullPage=Clubs">View All <img src="images/arrow_yellow.png" alt=""></a>
							<div class="clear"></div>
							<div class="listingTitle">
