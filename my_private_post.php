...
$Obj = new Query($DBName);
 ...
 $shoutsql="select forum.added_on,forum.forum_id, forum.forum,forum.forum_img,forum.forum_video,forum.status,user.first_name,user.last_name,user.image_nm from forum 
		  ...
		  where forum.status ='1' AND forum.user_id=".$_SESSION['user_id']."  AND forum_type!='regular' AND forum_type!='public' order by forum_id DESC";
 $shout_list = mysql_query($shoutsql);
	...
	if(count($_POST['shoutschk']) > 0)
		{
			  ...
			  {	
				  ...
				  header('location: my_private_post.php'); exit;
			  }
		}
	...
	<div class="main_home">
       ...
       <h2>Manage Clique Post </h2>
     		 ...
     		 <? if(isset($_SESSION['popup_update_post'])){?>
				  <div id="successmessage" style="margin-bottom:6px;"> Clique Post Successfully Updated</div> 
       ...
       <div id="profile_box">
     		...
     		<a  class="button addpost" onClick="javascript:void window.open('add_private_post_clique.php','','width=500,height=700,resizable=true,left=0,top=0');return false;"  style="font-size: 14px; float:right;">Add Post</a><?php } ?>			  
...
<input onclick="location.href = 'private_zone.php';" type="button" value="Back" class="button button" style="float: right;  margin-top: 10px;">
...
function delrecoreds(id)
{
  if(confirm('Are You sure You want to delete this record'))
  {
	 $.get( "deletepost.php?id="+id, function( data ) {
			window.location='my_private_post.php';
		});
  }
   else
   {
