					...
					<div id="calendar">
						...
						<?php $hid = $_GET['host_id']; ?>
						
						<div style="float: left; margin: 10px 0;">
						  	See all list of <a style="font-style: italic; color: rgb(254, 205, 7);" href="listevent.php?host_id=<?php echo $hid; ?>">Upcoming Events</a>
					  	</div>						
						<!-- VENUE CALENDAR -->
...
//echo "<pre>";print_r($events);die;
	...
	for($list_day = 1; $list_day <= $days_in_month; $list_day++):
		...
		$calendar.= '<div class="day-number"><a href="viewDateEvents.php?host_id='.$_GET['host_id'].'&date='.$date.'">'.$list_day.'</a></div>';
								  ...
								  //$calendar.= '<div class="event"><a href="read_event.php?id='.$event['id'].'">'.$stringlocation.'</a></div>';
									...
									if($userType=="user")
									{
										 ...
										 if($i<2){
										 	...
										 	{
												$i=0;
                                                    foreach($bookingsx  as $keysb=>$valueb ){
														
														
														if($valueb['date']==$event['date'] && $i<2){
                                                    ...
                                                    foreach($new_eventsx  as $keysx=>$valuex ){
														...
														if($valuex['date']==$event['date'] && $i<2){
															if(isset($event['eventname']))
																...
																$evntdate=date('M d,  Y',strtotime($valuex['date'])); 
															$calendar.= "<a class='tooltip' href='read_event.php?host_id=".$_GET['host_id']."&id=".$valuex['id']."'>".$stringlocation."</br>
											   			<span class='event'><b>Event Name : </b>".$valuex['eventname']."</br><b>Date : </b>".$evntdate."</span> </a>";
														if($i==2){?>
															<a href="listevent.php?host_id=<? echo $_GET['host_id'];?>">view all</a>
														<?}
														$i++;
															}
											   ...
											   $calendar.="<a class='read_mor' href='listevent.php?host_id=".$_GET['host_id']."'>View all</a>";
										   }
                                                    ...
                                                    foreach($new_eventsx  as $keysx=>$valuex ){
														...
														if($valuex['date']==$event['date'] && $i<2){
															if(isset($event['eventname']))
																...
																$evntdate=date('M d,  Y',strtotime($valuex['date'])); 
															$calendar.= "<a class='tooltip' href='read_event.php?id=".$event['id']."'>".$stringlocation."
											<span class='event'><b>Event Name : </b>".$valuex['eventname']."</br><b>Date : </b>".$evntdate."</span> </a><br /><br />";
											if($i==2){?>
															<a href="listeven.php?host_id=<? echo $_GET['host_id'];?>">view all</a>
														<?}
														$i++;
															}
...
/* "next month" control */
$next_month_link = '<a href="?host_id='.$_GET['host_id'].'&month='.($month != 12 ? $month + 1 : 1).'&year='.($month != 12 ? $year : $year + 1).'" class="control calendarright"><img src="images/arrow_right.png" alt="" /></a>';

/* "previous month" control */
$previous_month_link = '<a href="?host_id='.$_GET['host_id'].'&month='.($month != 1 ? $month - 1 : 12).'&year='.($month != 1 ? $year : $year - 1).'" class="control"><img src="images/arrow_left.png" alt="" /></a>';
...
$query = "SELECT eventname, date, id FROM events WHERE month(date) = '$month' AND year(date) = '$year' AND host_id='$hostID'";
...
$bookings_query = "SELECT id, book_type as eventname, requeted_date as date FROM bookings WHERE host_id='$hostID' AND status IN ('Accept','Confirm','Done')   ";
$result = mysql_query($query) or die('cannot get results!');
